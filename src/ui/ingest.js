/**
 * Telemetry ingestion (DOM layer).
 *
 * Shared by the file upload wiring and the in-app demo picker: takes raw file
 * text, routes it to the correct parser using header-based format detection, and
 * renders the results into their panels so the leaderboard, charts, and track
 * map coexist. Keeping one path means the demo exercises exactly the same code
...
/**
 * Routes telemetry text to the matching parser and updates the UI.
 *
 * @param {string} text - Raw file contents.
 * @param {string} name - File name, used for format detection and status text.
 * @returns {Promise<'race'|'trace'|'unknown'>} What was rendered.
 */
export async function ingestTelemetry(text, name) {
  const format = detectTelemetryFormat(text, name);

  switch (format) {
    case 'gpx':
    case 'gps-csv':
      renderGpsTelemetry(text, format, name);
      return 'trace';
    case 'lap-csv':
      await renderLapTelemetry(text, name);
      return 'race';
    default:
      setStatus(
        `Unrecognized file: ${name}. Upload a lap CSV (Driver, Lap, Time) or a GPS trace (.gpx, or CSV with Lat, Lon, Time).`,
        true
      );
      return 'unknown';
  }
...
export async function handleFile(file) {
  ...
  await ingestTelemetry(text, file.name);
}
